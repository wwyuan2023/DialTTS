...
class TextEncoder(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Sequential(
            nn.Linear(num_text, 128),
            nn.LayerNorm(128),
            LinearGLU(128, num_hidden),
...
class AdaTTS(nn.Module):
    def __init__(
        ...
        # text encoder
        self.text_encoder = TextEncoder(num_text, **text_encoder_params)
    ...
    def forward(self, spkid, text_padded, duration_padded, mel_padded, steps=0):
        ...
        assert N == text_padded.size(1), f"N={N}, text_padded={text_padded.size(1)}"
        
        text_mask = get_mask_from_lengths(text_lengths, N, text_padded.device).unsqueeze(2) # (B,N,1)
        mel_mask = get_mask_from_lengths(mel_lengths, T, text_padded.device).unsqueeze(2) # (B,T,1)
        
        # text encoding
        text_encoded = self.text_encoder(text_padded, self.positional_encoding, text_mask) # (B,N,c)
        ...
        # mel decoding
        mel_output1, mpg_output1 = self.mel_decoder(text_expanded, self.positional_encoding, mel_mask) # text_expanded: (B,T,d)
    ...
    def infer(self, spkid, text, duration_rate=1.0):
        ...
        # text encoding
        text_encoded = self.text_encoder(text, self.positional_encoding) # (B=1,N,c)
        ...
        # mel decoding
        _, mpg_output = self.mel_decoder(text_expanded, self.positional_encoding) # text_expanded: (B=1,T,d)
