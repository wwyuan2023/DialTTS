...
if [ "${n_gpus}" -gt 1 ]; then
    train="python3 -m dialtts.acoustic.distributed.launch --nproc_per_node ${n_gpus} --master_port ${master_port} -c dialtts-acoustic-train"
else
    train="dialtts-acoustic-train"
fi
